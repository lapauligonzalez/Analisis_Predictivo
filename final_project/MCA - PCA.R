# ---------- Analisis MCA- Cuando las variables son categóricas
...
#En este tipo de analisis se generan p-s ejes, donde p es el número de categorías y s el número de variables
#p = cantidad de elementos que nos devuelve la tabla de arriba
#s = cantidad de columnas
#por tanto serán 10 ejes (p = 16 - s = 6).

#2. ...

#Fijarme todo en :
#  https://rpubs.com/StivenQ/590164

# http://www.sthda.com/english/articles/31-principal-component-methods-in-r-practical-guide/114-mca-multiple-correspondence-analysis-in-r-essentials/


# ---------- Analisis PCA- Cuando las variables son numericas
...
base_datos<- base_datos[!duplicated(base_datos$Name), ] #sacamos los duplicados
base_datos<-textshape::column_to_rownames(base_datos, loc = 1) #pasamos los nombres de la oclumna 1, a los nombres de las filas

df<-as.data.frame(base_datos)
df <- subset(df, select = -c(Alignment,Total) ) #sacamos algunas columnas que no necesitamos
View(df)

#primera forma de hacer PCA: 
...
summary(respca) #aca vemos lo importante.  Vemos que en el primer y segundo componente explican mucha varianza.

#comprobemos la importancia del componente 1
xx<-respca$x #generamos un vector
xx<-as.data.frame(xx) #lo pasamos a dataframe
base_datos$PC1<-xx$PC1 #que nos pase el componente 1 a la base de datos como una variable
base_datos$PC2<-xx$PC2 #que nos pase el componente 2 a la base de datos como una variable
base_datoscor <- subset(base_datos, select = -c(Alignment)) # le sacamos una variable que no nos interesa
head(base_datoscor)
cor(base_datoscor)
#otra función
#princomp() 
